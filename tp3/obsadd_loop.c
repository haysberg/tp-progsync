/********
* ec2c version 0.68
* c main file generated for node : obsadd 
...
/* Output procedures **********************/
#ifdef CKCHECK
void obsadd_BOT_O(void* cdata){
   _put_bottom("O");
}
#endif
/* Output procedures **********************/
void obsadd_O_O(void* cdata, _boolean _V) {
   _put_bool("O", _V);
}/* Main procedure *************************/
int main(){
   ...
   int s = 0;
   /* Context allocation */
   struct obsadd_ctx* ctx = obsadd_new_ctx(NULL);
   obsadd_reset(ctx);
   /* Main loop */
   ISATTY = isatty(0);
   while(1){
      ...
      fflush(stdout);
      ++s;
      obsadd_I_a(ctx, _get_bool("a"));
      obsadd_I_b(ctx, _get_bool("b"));
      obsadd_I_c(ctx, _get_bool("c"));
      obsadd_step(ctx);
