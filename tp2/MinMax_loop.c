/********
* ec2c version 0.68
* c main file generated for node : MinMax 
...
/* Output procedures **********************/
#ifdef CKCHECK
void MinMax_BOT_min(void* cdata){
   _put_bottom("min");
}
void MinMax_BOT_max(void* cdata){
   _put_bottom("max");
}
#endif
/* Output procedures **********************/
void MinMax_O_min(void* cdata, _integer _V) {
   _put_int("min", _V);
}
void MinMax_O_max(void* cdata, _integer _V) {
   _put_int("max", _V);
}/* Main procedure *************************/
int main(){
   ...
   int s = 0;
   /* Context allocation */
   struct MinMax_ctx* ctx = MinMax_new_ctx(NULL);
   MinMax_reset(ctx);
   /* Main loop */
   ISATTY = isatty(0);
   while(1){
      ...
      fflush(stdout);
      ++s;
      MinMax_I_input(ctx, _get_int("input"));
      MinMax_step(ctx);
