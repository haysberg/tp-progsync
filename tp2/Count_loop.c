/********
* ec2c version 0.68
* c main file generated for node : Count 
...
/* Output procedures **********************/
#ifdef CKCHECK
void Count_BOT_out(void* cdata){
   _put_bottom("out");
}
#endif
/* Output procedures **********************/
void Count_O_out(void* cdata, _integer _V) {
   _put_int("out", _V);
}/* Main procedure *************************/
int main(){
   ...
   int s = 0;
   /* Context allocation */
   struct Count_ctx* ctx = Count_new_ctx(NULL);
   Count_reset(ctx);
   /* Main loop */
   ISATTY = isatty(0);
   while(1){
