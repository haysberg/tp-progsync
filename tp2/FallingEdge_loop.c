/********
* ec2c version 0.68
* c main file generated for node : FallingEdge 
...
/* Output procedures **********************/
#ifdef CKCHECK
void FallingEdge_BOT_falling(void* cdata){
   _put_bottom("falling");
}
#endif
/* Output procedures **********************/
void FallingEdge_O_falling(void* cdata, _boolean _V) {
   _put_bool("falling", _V);
}/* Main procedure *************************/
int main(){
   
   int s = 0;
   /* Context allocation */
   struct FallingEdge_ctx* ctx = FallingEdge_new_ctx(NULL);
   ...
   while(1){
      ...
      fflush(stdout);
      ++s;
      FallingEdge_I_val(ctx, _get_bool("val"));
      FallingEdge_step(ctx);
