/********
* ec2c version 0.68
* c main file generated for node : Latch 
...
/* Output procedures **********************/
#ifdef CKCHECK
void Latch_BOT_running(void* cdata){
   _put_bottom("running");
}
#endif
/* Output procedures **********************/
void Latch_O_running(void* cdata, _boolean _V) {
   _put_bool("running", _V);
}/* Main procedure *************************/
int main(){
   
   int s = 0;
   /* Context allocation */
   struct Latch_ctx* ctx = Latch_new_ctx(NULL);
   Latch_reset(ctx);
   /* Main loop */
   ISATTY = isatty(0);
   while(1){
      ...
      fflush(stdout);
      ++s;
      Latch_I_on(ctx, _get_bool("on"));
      Latch_I_off(ctx, _get_bool("off"));
      Latch_step(ctx);
