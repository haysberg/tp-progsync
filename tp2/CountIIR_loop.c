/********
* ec2c version 0.68
* c main file generated for node : CountIIR 
...
/* Output procedures **********************/
#ifdef CKCHECK
void CountIIR_BOT_out(void* cdata){
   _put_bottom("out");
}
#endif
/* Output procedures **********************/
void CountIIR_O_out(void* cdata, _integer _V) {
   _put_int("out", _V);
}/* Main procedure *************************/
int main(){
   ...
   int s = 0;
   /* Context allocation */
   struct CountIIR_ctx* ctx = CountIIR_new_ctx(NULL);
   CountIIR_reset(ctx);
   /* Main loop */
   ISATTY = isatty(0);
   while(1){
      ...
      fflush(stdout);
      ++s;
      CountIIR_I_init(ctx, _get_int("init"));
      CountIIR_I_incr(ctx, _get_int("incr"));
      CountIIR_I_reset(ctx, _get_bool("reset"));
      CountIIR_step(ctx);
