/********
* ec2c version 0.68
* c main file generated for node : CountR 
...
/* Output procedures **********************/
#ifdef CKCHECK
void CountR_BOT_out(void* cdata){
   _put_bottom("out");
}
#endif
/* Output procedures **********************/
void CountR_O_out(void* cdata, _integer _V) {
   _put_int("out", _V);
}/* Main procedure *************************/
int main(){
   ...
   int s = 0;
   /* Context allocation */
   struct CountR_ctx* ctx = CountR_new_ctx(NULL);
   CountR_reset(ctx);
   /* Main loop */
   ISATTY = isatty(0);
   while(1){
      ...
      fflush(stdout);
      ++s;
      CountR_I_inp(ctx, _get_bool("inp"));
      CountR_step(ctx);
