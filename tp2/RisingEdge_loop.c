/********
* ec2c version 0.68
* c main file generated for node : RisingEdge 
...
/* Output procedures **********************/
#ifdef CKCHECK
void RisingEdge_BOT_rising(void* cdata){
   _put_bottom("rising");
}
#endif
/* Output procedures **********************/
void RisingEdge_O_rising(void* cdata, _boolean _V) {
   _put_bool("rising", _V);
}/* Main procedure *************************/
int main(){
   ...
   int s = 0;
   /* Context allocation */
   struct RisingEdge_ctx* ctx = RisingEdge_new_ctx(NULL);
   ...
   while(1){
      ...
      fflush(stdout);
      ++s;
      RisingEdge_I_val(ctx, _get_bool("val"));
      RisingEdge_step(ctx);
