/********
* ec2c version 0.68
* c main file generated for node : XOR1bit 
...
/* Output procedures **********************/
#ifdef CKCHECK
void XOR1bit_BOT_out(void* cdata){
   _put_bottom("out");
}
#endif
/* Output procedures **********************/
void XOR1bit_O_out(void* cdata, _boolean _V) {
   _put_bool("out", _V);
}/* Main procedure *************************/
int main(){
   ...
   int s = 0;
   /* Context allocation */
   struct XOR1bit_ctx* ctx = XOR1bit_new_ctx(NULL);
   XOR1bit_reset(ctx);
   /* Main loop */
   ISATTY = isatty(0);
   while(1){
      ...
      fflush(stdout);
      ++s;
      XOR1bit_I_X(ctx, _get_bool("X"));
      XOR1bit_I_Y(ctx, _get_bool("Y"));
      XOR1bit_step(ctx);
