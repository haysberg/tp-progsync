/********
* ec2c version 0.68
* c main file generated for node : multiplexeur1bit 
* to be used with : multiplexeur1bit.c 
* and             : multiplexeur1bit.h 
* context   method = HEAP
...
/* Output procedures **********************/
#ifdef CKCHECK
void multiplexeur1bit_BOT_out(void* cdata){
   _put_bottom("out");
}
#endif
/* Output procedures **********************/
void multiplexeur1bit_O_out(void* cdata, _boolean _V) {
   _put_bool("out", _V);
}/* Main procedure *************************/
int main(){
   ...
   int s = 0;
   /* Context allocation */
   struct multiplexeur1bit_ctx* ctx = multiplexeur1bit_new_ctx(NULL);
   ...
   while(1){
      ...
      fflush(stdout);
      ++s;
      multiplexeur1bit_I_a(ctx, _get_bool("a"));
      multiplexeur1bit_I_b(ctx, _get_bool("b"));
      multiplexeur1bit_I_sel(ctx, _get_bool("sel"));
      multiplexeur1bit_step(ctx);
