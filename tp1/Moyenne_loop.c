/********
* ec2c version 0.68
* c main file generated for node : Moyenne 
...
/* Output procedures **********************/
#ifdef CKCHECK
void Moyenne_BOT_moyenne(void* cdata){
   _put_bottom("moyenne");
}
#endif
/* Output procedures **********************/
void Moyenne_O_moyenne(void* cdata, _integer _V) {
   _put_int("moyenne", _V);
}/* Main procedure *************************/
int main(){
   ...
   int s = 0;
   /* Context allocation */
   struct Moyenne_ctx* ctx = Moyenne_new_ctx(NULL);
   Moyenne_reset(ctx);
   /* Main loop */
   ISATTY = isatty(0);
   while(1){
      ...
      fflush(stdout);
      ++s;
      Moyenne_I_input(ctx, _get_int("input"));
      Moyenne_step(ctx);
