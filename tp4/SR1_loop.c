/********
* ec2c version 0.68
* c main file generated for node : SR1 
...
/* Output procedures **********************/
#ifdef CKCHECK
void SR1_BOT_Q(void* cdata){
   _put_bottom("Q");
}
#endif
/* Output procedures **********************/
void SR1_O_Q(void* cdata, _boolean _V) {
   _put_bool("Q", _V);
}/* Main procedure *************************/
int main(){
   ...
   int s = 0;
   /* Context allocation */
   struct SR1_ctx* ctx = SR1_new_ctx(NULL);
   SR1_reset(ctx);
   /* Main loop */
   ISATTY = isatty(0);
   while(1){
      ...
      fflush(stdout);
      ++s;
      SR1_I_init(ctx, _get_bool("init"));
      SR1_I_S(ctx, _get_bool("S"));
      SR1_I_R(ctx, _get_bool("R"));
      SR1_step(ctx);
